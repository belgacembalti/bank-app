...
import { createVirtualCard, VirtualCard } from '../../services/api';

interface VirtualCardGeneratorProps {
  onComplete: () => void;
  onCreated?: (card: VirtualCard) => void;
}

export function VirtualCardGenerator({ onComplete, onCreated }: VirtualCardGeneratorProps) {
  const [isGenerating, setIsGenerating] = useState(false);
  const [cardGenerated, setCardGenerated] = useState(false);
  const [cardData, setCardData] = useState<VirtualCard | null>(null);
  ...
  const generateCard = async () => {
    ...
    try {
      ...
      setError(err?.message || 'Unable to generate card. Please try again.');
    } finally {
    ...
    <div className="space-y-6">
      {isGenerating ? (
            ...
            <div className="bg-gradient-to-br from-primary-500 to-primary-700 rounded-2xl p-8 text-white shadow-2xl">
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <p className="text-2xl tracking-wider">{cardData.cardNumber}</p>
                </div>
        ...
        <div className="space-y-4">
          {error && (
            <div className="p-4 bg-danger-50 dark:bg-danger-900/20 border border-danger-200 dark:border-danger-800 rounded-lg text-danger-700 dark:text-danger-300">
              {error}
            </div>
          )}
          <Button variant="primary" fullWidth onClick={generateCard} loading={isGenerating}>
            Retry Generation
          </Button>
        </div>
      )}
