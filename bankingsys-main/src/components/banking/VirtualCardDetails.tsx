...
import { CreditCard, Lock, Unlock, MapPin, Activity } from 'lucide-react';
...
export function VirtualCardDetails() {
  ...
  useEffect(() => {
    const loadCards = async () => {
      ...
      setError(null);
      try {
        const fetched = await getVirtualCards();
        setCards(fetched);
      } catch (err: any) {
        setError(err?.message || 'Unable to load virtual cards');
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        {error && (
          <p className="text-sm text-danger-600 dark:text-danger-400 mt-2">{error}</p>
        )}
      </div>
          ...
          <div className="space-y-6">
            <div>
              ...
              <p className="text-2xl tracking-wider">
                {activeCard ? activeCard.cardNumber : 'No card generated yet'}
              </p>
            </div>
            <div className="flex gap-8">
              ...
              <div>
                ...
                <p className="text-lg">{activeCard ? activeCard.cvv : '•••'}</p>
              </div>
              <div>
                <p className="text-primary-100 text-xs mb-1">Token ID</p>
                <p className="text-lg">{activeCard ? activeCard.tokenId : 'N/A'}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <Card className="bg-danger-50 dark:bg-danger-900/20 border-danger-200 dark:border-danger-800">
        ...
        <Button variant="danger" disabled={!activeCard}>
          Delete Virtual Card
        </Button>
      </Card>
    </div>
