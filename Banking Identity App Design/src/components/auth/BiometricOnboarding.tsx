          ...
          <Button
            ...
            onClick={async () => {
              if (step === 2 && !isCapturing) {
                ...
                // Save biometric data on final step
                setSaving(true);
                setError('');
                try {
                  // In a real app, this would be actual encrypted biometric data
                  // For demo purposes, we're using a placeholder
                  const encryptedData = `encrypted_biometric_template_${Date.now()}`;
                  const response = await apiClient.saveBiometricData(encryptedData);
                  
                  if (response.data) {
                    onNavigate('dashboard');
                  } else {
                    setError(response.error || 'Failed to save biometric data');
                    setSaving(false);
                  }
                } catch (err) {
